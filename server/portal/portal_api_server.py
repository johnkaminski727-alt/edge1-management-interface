...
from carrier_operational import ENDPOINT_SCOPES, identity_from_config, operational_response
from carrier_review import REVIEW_READ_SCOPE, REVIEW_WRITE_SCOPE, ReviewValidationError
from carrier_review_api import (
...
REVIEW_LOG = Path("/var/lib/wwcx-portal/carrier_review_events.jsonl")
...
INTERNAL_GET_ROUTES = {REVIEW_QUEUE_ENDPOINT: REVIEW_READ_SCOPE}
INTERNAL_POST_ROUTES = {REVIEW_ACTION_ENDPOINT: REVIEW_WRITE_SCOPE}
...
def audit_event(client, endpoint, response_code, carrier_id=None, scope=None, reason=None, resource_id=None):
...
def authenticate_request(headers, body=b""):
    ...
    if not policy.get("enabled"):
        ...
        return identity_from_config(client, client_config) if client_config else None
    ...
    expected = hmac.new(
        client_config["secret"].encode(), signature_payload.encode(), hashlib.sha256
    ).hexdigest()
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        required_scope = INTERNAL_GET_ROUTES.get(self.path) or ENDPOINT_SCOPES.get(self.path)
        if identity is None:
        ...
        if self.path == REVIEW_QUEUE_ENDPOINT:
            try:
                payload = review_queue_response(identity, TICKET_LOG, CHANGE_LOG, REVIEW_LOG)
            except ReviewAuthorizationError as error:
                audit_event(identity.client_id, self.path, 403, carrier_id=identity.carrier_id, scope=required_scope, reason=str(error))
                self.send_error_json(403, str(error))
                return
            except ReviewValidationError as error:
                audit_event(identity.client_id, self.path, 503, scope=required_scope, reason=str(error))
                self.send_error_json(503, "review records unavailable")
                return
        ...
        if self.path in ENDPOINT_SCOPES:
            try:
                ...
                audit_event(identity.client_id, self.path, 503, carrier_id=identity.carrier_id, scope=required_scope, reason="operational_export_unavailable")
                self.send_error_json(503, "operational export unavailable")
                return
            self.send_payload(identity, payload, required_scope)
            return

        audit_event(identity.client_id, self.path, 404, carrier_id=identity.carrier_id, reason="endpoint_not_found")
        self.send_error_json(404, "endpoint not found")

    def do_POST(self):
        required_scope = WORKFLOW_ROUTES.get(self.path) or INTERNAL_POST_ROUTES.get(self.path)
        client_id = self.headers.get("X-Portal-Client", "unknown")
        ...
        try:
            ...
            audit_event(identity.client_id, self.path, 400, carrier_id=identity.carrier_id, scope=required_scope, reason="invalid_json")
            self.send_error_json(400, "invalid JSON body")
            return
        except ValueError as error:
            audit_event(identity.client_id, self.path, 400, carrier_id=identity.carrier_id, scope=required_scope, reason=str(error))
            self.send_error_json(400, str(error))
            return

        if self.path == REVIEW_ACTION_ENDPOINT:
            try:
                record = create_review_event(identity, REVIEW_LOG, TICKET_LOG, CHANGE_LOG, payload)
            except ReviewAuthorizationError as error:
                audit_event(identity.client_id, self.path, 403, carrier_id=identity.carrier_id, scope=required_scope, reason=str(error))
                self.send_error_json(403, str(error))
                return
            except ReviewValidationError as error:
                audit_event(identity.client_id, self.path, 400, scope=required_scope, reason=str(error))
                self.send_error_json(400, str(error))
                return
            resource_id = record["review_event_id"]
            audit_event(identity.client_id, self.path, 201, scope=required_scope, resource_id=resource_id)
            self.send_json(201, record)
            return

        if not identity.carrier_id:
            audit_event(identity.client_id, self.path, 403, scope=required_scope, reason="carrier identity is required")
            self.send_error_json(403, "carrier identity is required")
            return
        if required_scope not in identity.scopes:
            audit_event(identity.client_id, self.path, 403, carrier_id=identity.carrier_id, scope=required_scope, reason="scope denied")
        ...
        try:
            ...
            self.send_error_json(400, str(error))
            return
        audit_event(identity.client_id, self.path, 201, carrier_id=identity.carrier_id, scope=required_scope, resource_id=resource_id)
        self.send_json(201, record)
    ...
    def send_payload(self, identity, payload, required_scope):
        audit_event(identity.client_id, self.path, 200, carrier_id=identity.carrier_id, scope=required_scope)
    ...
    def send_json(self, status, payload):
        self.send_bytes(status, json.dumps(payload, sort_keys=True).encode("utf-8"))
