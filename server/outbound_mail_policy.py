...
def validate_policy(p:dict[str,Any])->None:
    ...
    if t["hidden_open_tracking"]: raise ValueError("hidden open tracking is prohibited")
    if t["device_fingerprinting"]: raise ValueError("device fingerprinting is prohibited")
    if t["collect_full_ip"]: raise ValueError("full IP-address storage is prohibited by this contract")
