...
class Edge1SecurityAuthHttpAdapter(SecurityHttpActionMixin, SecurityHttpHelpersMixin):
    def __init__(
        ...
        *,
        ...
        control_surfaces_path: Optional[Path] = None,
        control_surfaces_registry_path: Optional[Path] = None,
        now: Any = time.time,
    ):
        self.config = config
        ...
        self.control_surfaces_path = control_surfaces_path
        self.control_surfaces_registry_path = control_surfaces_registry_path
        self.now = now
    ...
    def handle(self, request: HttpRequest) -> HttpResponse:
        try:
            ...
            if request.path == route["control_surfaces"]:
                return self._method(request, {"GET"}, self._control_surfaces)
            if request.path == route["control_surfaces_registry"]:
                return self._method(request, {"GET"}, self._control_surfaces_registry)
    ...
    def _method(self, request: HttpRequest, allowed: set[str], handler: Any) -> HttpResponse:
        if request.method.upper() not in allowed:
            ...
            return dataclasses.replace(response, headers=response.headers + (("Allow", ", ".join(sorted(allowed))),))
        return handler(request)

    def _health(self, request: HttpRequest) -> HttpResponse:
        return self._json(200, {"status": "ok", "live_route_authorized": self.config.live_route_authorized, "mutations_enabled": False})

    def _authenticated_context(self, request: HttpRequest):
    ...
    def _render_html_file(self, request: HttpRequest, path: Optional[Path]) -> HttpResponse:
        if self._authenticated_context(request) is None:
        ...
        rendered = source.replace("<style>", f'<style nonce="{nonce}">', 1).replace("<script>", f'<script nonce="{nonce}">', 1)
        ...
        headers = (
            ...
            ("Content-Security-Policy", "default-src 'self'; " + f"script-src 'nonce-{nonce}'; style-src 'nonce-{nonce}'; " + "connect-src 'self'; img-src 'self' data:; object-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'"),
        )
        return HttpResponse(200, headers, body)

    def _console(self, request: HttpRequest) -> HttpResponse:
        return self._render_html_file(request, self.console_path)

    def _control_surfaces(self, request: HttpRequest) -> HttpResponse:
        return self._render_html_file(request, self.control_surfaces_path)

    def _control_surfaces_registry(self, request: HttpRequest) -> HttpResponse:
        if self._authenticated_context(request) is None:
            return self._json(429, {"error": "rate_limited"})
        path = self.control_surfaces_registry_path
        if path is None or path.is_symlink() or not path.is_file():
            raise GatewayError("control_surfaces_registry_unavailable")
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise GatewayError("control_surfaces_registry_unavailable") from exc
        if not isinstance(payload, dict) or payload.get("contract") != "wwcx.edge1-control-surfaces.v1":
            raise GatewayError("control_surfaces_registry_invalid")
        response = self._json(200, payload)
        return dataclasses.replace(response, headers=response.headers + (("Cache-Control", "no-store, max-age=0"),))
    ...
    def _exchange(self, request: HttpRequest) -> HttpResponse:
        ...
        if not self.gateway.store.allow_rate("exchange:" + remote_key, int(self.now()), limit=self.config.exchange_requests, window_seconds=self.config.exchange_window_seconds):
            return self._json(429, {"error": "rate_limited"})
        try:
        ...
        self.gateway.store.set_csrf(context.session_identifier_hash, hash_secret(csrf_token), context.expires_at)
        headers = self._base_headers() + (("Location", self.config.routes["redirect_after_exchange"]), ("Set-Cookie", self._session_cookie(session_token)), ("Set-Cookie", self._csrf_cookie(csrf_token)))
    ...
    def _session(self, request: HttpRequest) -> HttpResponse:
        ...
        if CONSOLE_READ_SCOPE not in context.scopes:
            raise AuthorizationError("console_scope_required")
        if not self.gateway.store.allow_rate("session:" + context.session_identifier_hash, int(self.now()), limit=self.config.session_requests, window_seconds=self.config.session_window_seconds):
            return self._json(429, {"error": "rate_limited"})
        return self._json(200, {"authenticated": True, "display_name": context.display_name, "source_role": context.source_role, "scopes": sorted(context.scopes), "expires_at": context.expires_at, "authentication_event_id": context.authentication_event_id})

    def _logout(self, request: HttpRequest) -> HttpResponse:
        self._require_same_origin(request)
        token = self._session_token(request)
        context = self.gateway.authenticate_session(token, self._request_id())
        if not self.gateway.store.allow_rate("logout:" + context.session_identifier_hash, int(self.now()), limit=self.config.logout_requests, window_seconds=self.config.logout_window_seconds):
            return self._json(429, {"error": "rate_limited"})
        self._require_csrf(request, context.session_identifier_hash)
        self.gateway.logout(token, self._request_id())
        return HttpResponse(204, self._base_headers() + (("Set-Cookie", self._clear_cookie(self.config.session_cookie_name, http_only=True)), ("Set-Cookie", self._clear_cookie(self.config.csrf_cookie_name, http_only=False))), b"")


__all__ = ["Edge1SecurityAuthHttpAdapter", "HttpAdapterConfig", "HttpRequest", "HttpResponse"]
