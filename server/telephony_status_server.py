...
def service_active(name: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            check=False, timeout=1.5, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
        )
...
def service_record(name: str, role: str, port: int | None = None, health_url: str | None = None) -> dict[str, Any]:
    ...
    reachable, latency = (tcp_probe("127.0.0.1", port) if port else (active, None))
...
def asterisk_snapshot() -> tuple[int, int]:
    """Return active channel and endpoint counts when the local CLI is permitted."""
    try:
        channels = subprocess.run(
            ["asterisk", "-rx", "core show channels concise"], capture_output=True, text=True,
            timeout=2, check=False,
        )
        endpoints = subprocess.run(
            ["asterisk", "-rx", "pjsip show endpoints"], capture_output=True, text=True,
            timeout=2, check=False,
        )
        active_calls = sum(1 for line in channels.stdout.splitlines() if line.strip()) // 2
...
def status_payload() -> dict[str, Any]:
    ...
    services = [
        service_record("asterisk.service", "PBX and SIP application", 5060),
    ...
    critical = sum(1 for item in services if item["status"] == "critical")
    ...
    payload.update({
        ...
        "mode": "live_read_only",
        "site": socket.getfqdn(),
        "overall_status": "critical" if critical else ("degraded" if healthy < len(services) else "healthy"),
        "services": services,
    })
    metrics = dict(payload.get("metrics", {}))
    metrics.update({"active_calls": active_calls, "registrations": registrations, "critical_alerts": critical})
    payload["metrics"] = metrics
    payload["alerts"] = [
        {"severity": item["status"], "title": f"{item['name']} is {item['status']}",
         "summary": item["role"], "source": item["id"], "opened_at": payload["generated_at"]}
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path.split("?", 1)[0] == "/api/telephony/status":
