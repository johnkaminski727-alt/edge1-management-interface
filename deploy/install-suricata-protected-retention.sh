...
rollback() {
  ...
  if [[ -f "$EVIDENCE/service.before" ]]; then
    install -o root -g root -m 0644 "$EVIDENCE/service.before" "$SERVICE_DST"
  else
    rm -f "$SERVICE_DST"
  fi
  if [[ -f "$EVIDENCE/timer.before" ]]; then
    install -o root -g root -m 0644 "$EVIDENCE/timer.before" "$TIMER_DST"
  else
    rm -f "$TIMER_DST"
  fi
...
assert v["contract"] == "wwcx.suricata-protected-retention-policy.v1"
assert v["status"] == "live_authorized"
assert v["enabled"] is True
assert v["acceptance"]["deployment_authorized"] is True
assert v["acceptance"]["suricata_configuration_changed"] is False
assert v["acceptance"]["suricata_service_changed"] is False
assert v["acceptance"]["traffic_controls_changed"] is False
assert v["acceptance"]["authentication_boundary_changed"] is False
assert v["acceptance"]["public_access_changed"] is False
PY
...
p = pathlib.Path(sys.argv[1])
v = json.loads(p.read_text())
assert v["contract"] == "wwcx.suricata-protected-retention-status.v1"
assert v["state"] in {"healthy", "capacity_limited"}
assert v["public_access"] is False
assert v["network_listener"] is False
assert v["raw_eve_accessed"] is False
assert v["suricata_service_changed"] is False
assert v["traffic_controls_changed"] is False
PY

python3 - "$STATE_ROOT/alerts.sqlite3" <<'PY'
import os, pathlib, sqlite3, stat, sys
p = pathlib.Path(sys.argv[1])
assert p.is_file()
