...
function fetch_json(string $baseUrl, string $path, string $token): array {
    ...
    if ($body === false) throw new RuntimeException('Electrum API is unavailable');
    $status = 0;
    foreach ($http_response_header ?? [] as $header) {
        if (preg_match('/^HTTP\/\S+\s+(\d{3})/', $header, $match)) $status = (int)$match[1];
    }
    if ($status !== 200) throw new RuntimeException('Electrum API returned an error');
...
try {
    ...
    if ($baseUrl === '' || strlen($token) < 32) throw new RuntimeException('Electrum integration is not configured');
    $info = fetch_json($baseUrl, '/v1/wallet/info', $token);
    $balance = fetch_json($baseUrl, '/v1/wallet/balance', $token);
    respond(200, ['ok' => true, 'generated_at' => gmdate('c'), 'info' => $info, 'balance' => $balance]);
