...
if [ "$BRANCH" != main ]; then
    echo "apply requires a main-branch checkout" >&2
    exit 4
fi
if [ -n "$DIRTY" ]; then
    echo "apply requires a clean working tree" >&2
    exit 4
fi
...
if [ "$WAS_ACTIVE" -eq 1 ] && [ "$START" -ne 1 ]; then
    echo "active service upgrade requires --start so the validated unit is restarted" >&2
    exit 4
fi
...
rollback() {
    ...
    if [ "$HAD_CONFIG" -eq 1 ]; then
        cp -a "$EVIDENCE/config.before" "$CONFIG_TARGET"
    else
        rm -f "$CONFIG_TARGET"
    fi
    systemctl daemon-reload || true
    if [ "$WAS_ENABLED" -eq 1 ]; then
        systemctl enable edge1-comms-relay.service >/dev/null 2>&1 || true
    else
        systemctl disable edge1-comms-relay.service >/dev/null 2>&1 || true
    fi
    if [ "$WAS_ACTIVE" -eq 1 ]; then
        systemctl restart edge1-comms-relay.service || true
    else
        systemctl stop edge1-comms-relay.service >/dev/null 2>&1 || true
    fi
    exit "$rc"
}
...
if [ "$START" -eq 1 ]; then
    ...
    if [ "$WAS_ACTIVE" -eq 1 ]; then
        systemctl restart edge1-comms-relay.service
    else
        systemctl start edge1-comms-relay.service
    fi
    systemctl is-active --quiet edge1-comms-relay.service
    if python3 "$SMOKE" --config "$CONFIG_TARGET" --attempts 12 --delay 0.5 > "$EVIDENCE/smoke-test.txt" 2>&1; then
        cat "$EVIDENCE/smoke-test.txt"
    else
        cat "$EVIDENCE/smoke-test.txt" >&2
        exit 5
    fi
