...
if ! nft -c -f "$PERSIST"; then
    cp -a "$EVIDENCE_DIR/nftables.conf.before" "$PERSIST"
    fail "persistent nftables syntax check failed; original file restored"
fi
...
if ! nft -a list chain inet wwcxfw input | grep -Fq "$RULE_COMMENT"; then
    ...
    if [ -z "$WEB_HANDLE" ]; then
        cp -a "$EVIDENCE_DIR/nftables.conf.before" "$PERSIST"
        fail "could not locate live public-web rule handle; persistent file restored"
    fi
    ...
    if ! nft insert rule inet wwcxfw input position "$WEB_HANDLE" ip daddr "$PUBLIC_IP" udp dport 123 accept comment "$RULE_COMMENT"; then
        cp -a "$EVIDENCE_DIR/nftables.conf.before" "$PERSIST"
        fail "live NTP firewall insertion failed; persistent file restored"
    fi
    LIVE_INSERTED=1
fi
...
if ! nft -a list chain inet wwcxfw input | grep -Fq "$RULE_COMMENT"; then
    ...
    if [ "$LIVE_INSERTED" -eq 1 ]; then
        HANDLE=$(nft -a list chain inet wwcxfw input | awk -v c="$RULE_COMMENT" '$0 ~ c {for (i=1; i<=NF; i++) if ($i == "handle") {print $(i+1); exit}}')
        [ -n "$HANDLE" ] && nft delete rule inet wwcxfw input handle "$HANDLE" || true
    fi
    fail "live NTP firewall rule verification failed; attempted rollback"
fi

if ! grep -Fq "ip daddr $PUBLIC_IP udp dport 123 accept comment \"$RULE_COMMENT\"" "$PERSIST"; then
    fail "persistent NTP firewall rule verification failed"
fi
