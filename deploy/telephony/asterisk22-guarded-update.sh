...
[[ "$(hostname -f)" == "$EXPECTED_HOST" ]] || {
    echo "ERROR expected $EXPECTED_HOST, found $(hostname -f)" >&2
    exit 2
}

for command in asterisk apt-cache apt-get dpkg-query dpkg awk tar sha256sum; do
    command -v "$command" >/dev/null 2>&1 || { echo "ERROR missing $command" >&2; exit 2; }
done

channel_state="$(asterisk -rx 'core show channels count')"
printf '%s\n' "$channel_state"
active_channels="$(awk '/active channels/{value=$1} END{print value}' <<<"$channel_state")"
[[ "$active_channels" =~ ^[0-9]+$ ]] || { echo "ERROR cannot determine channel count" >&2; exit 3; }
[[ "$active_channels" -eq 0 ]] || { echo "ERROR active calls prevent update" >&2; exit 3; }
...
[[ -n "$candidate" && "$candidate" != "(none)" ]] || { echo "ERROR no candidate" >&2; exit 3; }
case "${candidate#*:}" in 22.*) ;; *) echo "ERROR candidate is not Asterisk 22" >&2; exit 3 ;; esac
...
((${#packages[@]})) || { echo "ERROR no installed asterisk22 packages" >&2; exit 3; }
printf 'Package: %s\n' "${packages[@]}"

simulation="$(mktemp)"
trap 'rm -f "$simulation"' EXIT
apt-get -s --only-upgrade install "${packages[@]}" | tee "$simulation"
...
mkdir -p "$EVID"
exec > >(tee -a "$EVID/update.log") 2>&1

echo "Evidence: $EVID"
dpkg-query -W -f='${binary:Package}\t${Version}\t${db:Status-Abbrev}\n' 'asterisk22*' >"$EVID/packages-before.txt" 2>/dev/null || true
backup_paths=(etc/asterisk)
[[ -e /var/lib/asterisk/astdb ]] && backup_paths+=(var/lib/asterisk/astdb)
tar -C / -czf "$EVID/asterisk-config-before.tgz" "${backup_paths[@]}"
sha256sum "$EVID/asterisk-config-before.tgz" >"$EVID/asterisk-config-before.tgz.sha256"
asterisk -rx 'core show version' >"$EVID/version-before.txt"
asterisk -rx 'pjsip show transports' >"$EVID/transports-before.txt"
ss -lntup | grep -E 'asterisk|kamailio|:5060|:5061|:5038|:8088|:8089' >"$EVID/listeners-before.txt" || true

apt-get update
refreshed_candidate="$(apt-cache policy asterisk22 | awk '/^[[:space:]]*Candidate:/{value=$2} END{print value}')"
[[ "$refreshed_candidate" == "$candidate" ]] || {
    echo "ERROR candidate changed after metadata refresh: $candidate -> $refreshed_candidate" >&2
    exit 5
}

channel_state="$(asterisk -rx 'core show channels count')"
active_channels="$(awk '/active channels/{value=$1} END{print value}' <<<"$channel_state")"
[[ "$active_channels" == "0" ]] || { echo "ERROR calls appeared before install" >&2; exit 5; }
...
expected="$(sed -E 's/^([0-9]+\.[0-9]+\.[0-9]+).*/\1/' <<<"${installed#*:}")"
echo "Installed package: $installed"
echo "Running binary: $running"
echo "Expected binary: $expected"
[[ "$running" == "$expected" ]] || { echo "ERROR running binary mismatch" >&2; exit 6; }
...
ss -lntup | grep -E 'asterisk|kamailio|:5060|:5061|:5038|:8088|:8089' | tee "$EVID/listeners-after.txt" || true
dpkg-query -W -f='${binary:Package}\t${Version}\t${db:Status-Abbrev}\n' 'asterisk22*' >"$EVID/packages-after.txt" 2>/dev/null || true

echo "ASTERISK UPDATE COMPLETED AND VALIDATED"
