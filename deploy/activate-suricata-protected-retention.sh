...
LIBEXEC=/usr/local/libexec/wwcx-security/suricata_protected_retention.py
...
backup "$LIBEXEC" libexec
backup "$POLICY" policy

install -d -o root -g root -m 0755 /usr/local/libexec/wwcx-security /etc/wwcx/security
install -d -o root -g root -m 0700 "$DATA_DIR"
install -o root -g root -m 0755 "$REPO_ROOT/server/suricata_protected_retention.py" "$LIBEXEC"
...
python3 "$LIBEXEC" --policy "$POLICY" verify | tee "$EVIDENCE_DIR/runtime-verification.json"
...
if value.get('accepted_count') != 0 or int(value.get('duplicate_count', 0)) < 0:
