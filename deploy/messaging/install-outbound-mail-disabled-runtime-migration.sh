...
case "$ACTION" in audit|install|verify|disable) ;; *) echo "ACTION must be audit, install, verify, or disable." >&2; exit 2 ;; esac
for value in "$REPO" "$SOURCE_CONFIG" "$SOURCE_IDENTITIES" "$CONFIG_ROOT" "$STATE_ROOT" "$RUNTIME_CONFIG" "$RUNTIME_POLICY" "$RUNTIME_IDENTITIES" "$RUNTIME_AUDIT" "$RUNTIME_NONCES" "$SUPPRESSION_DATABASE" "$DROPIN_DIR" "$DROPIN" "$EVIDENCE_ROOT"; do
    case "$value" in *" "*|*"	"*) echo "Configured paths must not contain whitespace." >&2; exit 1 ;; esac
done
...
record() { printf '%s=%s\n' "$1" "$2" | tee -a "$summary"; }
manifest() {
    (cd "$evidence_dir" && find . -type f ! -name SHA256SUMS -print | sort | xargs sha256sum > SHA256SUMS)
}
...
python3 - "$SOURCE_CONFIG" "$REPO" <<'PY' > "$evidence_dir/source-paths.json"
import json
import pathlib
import sys
...
def resolve(value):
    ...
    if path.is_absolute():
        raise SystemExit("source runtime paths must be repository-relative before migration")
...
PY
source_policy=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["policy"])' "$evidence_dir/source-paths.json")
source_audit=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["audit"])' "$evidence_dir/source-paths.json")
source_nonce=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["nonce"])' "$evidence_dir/source-paths.json")
...
python3 tools/messaging/build_outbound_mail_disabled_runtime_bundle.py \
    ...
    --output-dir "$bundle_dir" > "$evidence_dir/runtime-bundle-build.json"
...
validate_roots() {
    python3 - "$CONFIG_ROOT" "$STATE_ROOT" "$service_user" "$service_group" <<'PY'
import grp
import os
import pathlib
import pwd
...
uid = pwd.getpwnam(sys.argv[3]).pw_uid
gid = grp.getgrnam(sys.argv[4]).gr_gid
for path in (config, state):
    if not path.is_dir() or path.is_symlink():
        raise SystemExit(f"unsafe runtime root: {path}")
config_stat = config.stat()
state_stat = state.stat()
if config_stat.st_uid != 0 or stat.S_IMODE(config_stat.st_mode) & 0o022:
    raise SystemExit("config root must be root-owned and not group/world writable")
if state_stat.st_uid != uid or state_stat.st_gid != gid or stat.S_IMODE(state_stat.st_mode) & 0o027:
    raise SystemExit("state root owner/group/mode is unsafe")
PY
}
...
verify_live() {
    ...
    for file in "$RUNTIME_CONFIG" "$RUNTIME_POLICY" "$RUNTIME_IDENTITIES"; do
        [ -f "$file" ] && [ ! -L "$file" ] || return 1
        [ "$(stat -c '%U' "$file")" = root ] || return 1
        [ "$(stat -c '%a' "$file")" = 644 ] || return 1
    done
    [ -f "$RUNTIME_AUDIT" ] && [ ! -L "$RUNTIME_AUDIT" ] || return 1
    [ "$(stat -c '%U:%G:%a' "$RUNTIME_AUDIT")" = "$service_user:$service_group:600" ] || return 1
    if [ -e "$RUNTIME_NONCES" ]; then
        [ -f "$RUNTIME_NONCES" ] && [ ! -L "$RUNTIME_NONCES" ] || return 1
        [ "$(stat -c '%U:%G:%a' "$RUNTIME_NONCES")" = "$service_user:$service_group:600" ] || return 1
    fi
    [ "$(stat -c '%U:%G:%a' "$SUPPRESSION_DATABASE")" = "$service_user:$service_group:600" ] || return 1
    ...
    send_code=$(curl --silent --show-error --max-time 8 --header 'Content-Type: application/json' \
        ...
        --output "$evidence_dir/disabled-send.json" --write-out '%{http_code}' "http://$LISTEN_HOST:$LISTEN_PORT/outbound-mail/send")
...
PY
    ...
    if ss -ltnH | awk -v port=":$LISTEN_PORT" '$4 ~ port "$" {print $4}' | grep -Ev '^(127\.0\.0\.1|\[::1\]):' >/dev/null 2>&1; then return 1; fi
}

if [ "$ACTION" = audit ]; then
    current_exec=$(systemctl show "$SERVICE" -p ExecStart --value)
    case "$current_exec" in *outbound_mail_gateway_runtime_server.py*) state=installed ;; *) state=not_installed ;; esac
...
had_dropin=no
backup_dropin=$evidence_dir/40-runtime-paths.conf.before
if [ -e "$DROPIN" ]; then
    [ -f "$DROPIN" ] && [ ! -L "$DROPIN" ] || { echo "Runtime migration drop-in is unsafe." >&2; exit 1; }
...
config_root_created=no
state_root_created=no
dropin_dir_created=no
mutated=no
completed=no
restore_dropin() {
    if [ "$had_dropin" = yes ]; then
        ...
        mv "$DROPIN" "$evidence_dir/40-runtime-paths.conf.rolled-back"
    fi
}
rollback() {
    reason=$1
    ...
    [ "$dropin_dir_created" = yes ] && rmdir "$DROPIN_DIR" 2>/dev/null || true
    [ "$state_root_created" = yes ] && rmdir "$STATE_ROOT" 2>/dev/null || true
    [ "$config_root_created" = yes ] && rmdir "$CONFIG_ROOT" 2>/dev/null || true
    printf '%s\n' "$reason" >> "$failures"
...
if [ "$ACTION" = disable ]; then
    [ "$had_dropin" = yes ] || { echo "Runtime migration drop-in is absent." >&2; exit 1; }
    ...
    systemctl restart "$SERVICE"
    sleep 2
    systemctl is-active --quiet "$SERVICE" || rollback "prior gateway did not recover after disable"
    exec_start=$(systemctl show "$SERVICE" -p ExecStart --value)
    printf '%s' "$exec_start" | grep -Fv outbound_mail_gateway_runtime_server.py >/dev/null || rollback "runtime entrypoint remained active after disable"
    [ "$(sha256sum "$SOURCE_CONFIG" | awk '{print $1}')" = "$source_config_sha" ] || rollback "source configuration changed during disable"
...
[ "$had_dropin" = no ] || { echo "Runtime migration drop-in already exists; use verify or disable." >&2; exit 1; }
for destination in "$RUNTIME_CONFIG" "$RUNTIME_POLICY" "$RUNTIME_IDENTITIES" "$RUNTIME_AUDIT" "$RUNTIME_NONCES"; do
    [ ! -e "$destination" ] || { echo "Migration destination already exists: $destination" >&2; exit 1; }
done

mutated=yes
if [ ! -d "$CONFIG_ROOT" ]; then install -d -o root -g root -m 0755 "$CONFIG_ROOT"; config_root_created=yes; fi
if [ ! -d "$STATE_ROOT" ]; then install -d -o "$service_user" -g "$service_group" -m 0750 "$STATE_ROOT"; state_root_created=yes; fi
if [ ! -d "$DROPIN_DIR" ]; then install -d -o root -g root -m 0755 "$DROPIN_DIR"; dropin_dir_created=yes; fi
validate_roots || rollback "runtime root ownership or permissions are unsafe"
...
if [ ! -e "$SUPPRESSION_DATABASE" ]; then
    ...
    [ "$(stat -c '%U:%G:%a' "$SUPPRESSION_DATABASE")" = "$service_user:$service_group:600" ] || rollback "existing suppression database ownership or mode mismatch"
