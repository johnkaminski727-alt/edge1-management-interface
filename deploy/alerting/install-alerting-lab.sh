...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --expected-host)
            ...
            ;;
        --source-root)
            [ "$#" -ge 2 ] || { echo "ERROR missing source root" >&2; exit 2; }
...
required='tools/alerting/capcp_probe.py
...
sh -n "$SOURCE_ROOT/tools/alerting/asterisk_alerting_readiness.sh"
...
find "$TARGET" -type f -print0 | sort -z | xargs -0 sha256sum \
    >"$EVID/installed-sha256.txt"
"$TARGET/bin/capcp_probe.py" "$TARGET/fixtures/capcp-test-alert.xml" \
    >"$EVID/capcp-smoke.json"
"$TARGET/bin/capcp_lifecycle_probe.py" "$TARGET/fixtures/capcp-test-alert.xml" \
    >"$EVID/lifecycle-smoke.json"
"$TARGET/bin/asterisk_alerting_readiness.sh" --expected-host "$EXPECTED_HOST" \
    >"$EVID/asterisk-readiness.txt" 2>&1 || true
