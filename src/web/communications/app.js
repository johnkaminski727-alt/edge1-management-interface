(() => {
  ...
  function renderInspector(event) {
    ...
    const rows = [
      ["Event", event.communications_event_id], ["Conversation", event.conversation_id], ["Thread", event.thread_id],
      ["Case", event.case_id], ["Channel", channelLabel(event.channel)], ["Direction", event.direction], ["State", event.status],
      ["Sender identity", event.sender_identity_ref], ["Recipients", (event.recipient_identity_refs || []).join(", ")],
      ["Security", event.security && event.security.state], ["Native source", event.native_record && event.native_record.source],
      ["Native record", event.native_record && event.native_record.record_id], ["Provider", event.native_record && event.native_record.provider],
      ["AI derived", event.derived && event.derived.ai_generated ? "yes" : "no"], ["Audit", (event.audit_refs || []).join(", ")],
    ];
  ...
  async function loadEvents() {
    ...
    try {
      ...
      if (payload.content_is_untrusted !== true) throw new Error("content_is_untrusted contract was not preserved");
  ...
  async function loadReadiness() {
    ...
    try {
      ...
      for (const [channel, values] of Object.entries(payload.channels || {})) {
        const card = document.createElement("article"); card.className = "readiness-channel";
        const heading = document.createElement("h3"); heading.textContent = channel.replaceAll("_", " / "); card.append(heading);
        for (const key of ["repository_implementation", "edge1_runtime", "private_ai_adapter", "identity_mapping", "security_quarantine", "provider_configuration", "production_authorization", "live_acceptance", "rollback_evidence"]) {
          const row = document.createElement("div"); const label = document.createElement("span"); label.textContent = key.replaceAll("_", " ");
          row.append(label, readinessBadge(values[key])); card.append(row);
        }
        readinessGrid.append(card);
      }
    } catch (error) {
      const message = document.createElement("p"); message.className = "empty-state";
      message.textContent = `Readiness matrix unavailable: ${error.message}`; readinessGrid.append(message);
    }
  }

  document.querySelector("#search-form").addEventListener("submit", (event) => { event.preventDefault(); state.query = document.querySelector("#search-input").value.trim(); loadEvents(); });
  document.querySelectorAll("#channel-filters button").forEach((button) => button.addEventListener("click", () => { document.querySelectorAll("#channel-filters button").forEach((item) => item.classList.remove("active")); button.classList.add("active"); state.channel = button.dataset.channel || "all"; loadEvents(); }));
  document.querySelectorAll(".queue").forEach((button) => button.addEventListener("click", () => { document.querySelectorAll(".queue").forEach((item) => item.classList.remove("active")); button.classList.add("active"); state.queueState = button.dataset.state || ""; loadEvents(); }));
