...
const INSPECTIONS = {
  ...
  bridge: `snapshot=${shellQuote(BRIDGE_SNAPSHOT)}; if [ -f "$snapshot" ]; then stat -c 'path=%n\\nsize=%s\\nmtime_unix=%Y\\nmode=%a' "$snapshot"; if [ -f "$snapshot.sha256" ]; then echo 'checksum_sidecar=present'; sha256sum -c "$snapshot.sha256" 2>/dev/null && echo 'checksum=verified' || echo 'checksum=failed'; else echo 'checksum_sidecar=missing'; fi; else echo 'snapshot=missing'; exit 4; fi`,
