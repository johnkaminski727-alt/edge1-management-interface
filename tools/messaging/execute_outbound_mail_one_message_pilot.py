...
class RuntimeAdapter:
    ...
    def get_json(self, url: str) -> tuple[int, dict[str, Any]]:
        ...
        try:
            ...
            raise PilotExecutionError("loopback gateway health request failed") from exc

    def send_once(self, url: str, payload: dict[str, Any]) -> tuple[int, dict[str, Any]]:
...
def validate_private_file(path: pathlib.Path, label: str, *, owner_uid: int | None = None) -> pathlib.Path:
...
def validate_bundle_file(path: pathlib.Path, expected_sha256: str) -> pathlib.Path:
    absolute = validate_private_file(path, "activation bundle file")
...
def validate_request(request: dict[str, Any], authorization: dict[str, Any]) -> dict[str, Any]:
    expected = {"to", "subject", "body", "message_class", "identity_hint", "confirm_send"}
    if set(request) != expected:
        raise PilotExecutionError("pilot request keys are invalid")
    recipients = request["to"]
    if not isinstance(recipients, list) or len(recipients) != 1 or not isinstance(recipients[0], str):
    ...
    if not isinstance(request["subject"], str) or not request["subject"].strip():
        raise PilotExecutionError("pilot subject is missing")
    if not isinstance(request["body"], str) or not request["body"].strip():
        raise PilotExecutionError("pilot body is missing")
    ...
    if str(request["identity_hint"]).strip().casefold() != str(authorization["sender_address"]).casefold():
    ...
    if sha256_bytes(canonical_document_bytes(request)) != authorization["pilot_payload_sha256"]:
        raise PilotExecutionError("pilot request hash does not match authorization")
    if sha256_bytes(recipient.encode("utf-8")) != authorization["pilot_recipient_sha256"]:
        raise PilotExecutionError("pilot recipient hash does not match authorization")
    return {"request": request, "recipient": recipient}


def validate_runtime_documents(runtime_root: pathlib.Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, pathlib.Path]]:
    paths = {filename: validate_runtime_file(runtime_root / filename) for filename in RUNTIME_FILENAMES}
...
def validate_manifest(
    ...
    *,
    ...
    try:
        activation_bundle.validate_safe_sources(config, policy, identities)
        activation_bundle.validate_authorization(
            authorization,
            config,
            policy,
            identities,
    ...
    if manifest.get("authorization_sha256") != activation_bundle.sha256_document(authorization):
        raise PilotExecutionError("activation bundle authorization hash mismatch")
    expected_sources = {
        RUNTIME_FILENAMES[0]: activation_bundle.sha256_document(config),
        RUNTIME_FILENAMES[1]: activation_bundle.sha256_document(policy),
        RUNTIME_FILENAMES[2]: activation_bundle.sha256_document(identities),
    }
    if manifest.get("source_runtime_sha256") != expected_sources:
        raise PilotExecutionError("activation bundle source runtime hashes mismatch")
    if manifest.get("pilot_recipient_sha256") != authorization["pilot_recipient_sha256"]:
        raise PilotExecutionError("activation bundle recipient hash mismatch")
    if manifest.get("pilot_payload_sha256") != authorization["pilot_payload_sha256"]:
        raise PilotExecutionError("activation bundle payload hash mismatch")
    if manifest.get("smtp_auth_canary_sha256") != authorization["smtp_auth_canary_sha256"]:
        raise PilotExecutionError("activation bundle SMTP canary hash mismatch")
    if manifest.get("max_recipient_count") != 1 or manifest.get("rollback_required") is not True:
        raise PilotExecutionError("activation bundle is not bounded to one recipient with rollback")
    for marker in ("credentials_read", "runtime_files_modified", "provider_contacted", "message_prepared", "message_sent"):
        if manifest.get(marker) is not False:
            raise PilotExecutionError(f"activation bundle safety marker changed: {marker}")
    ...
    for relative, digest in hashes.items():
        ...
        files[relative] = validate_bundle_file(bundle_dir / relative, digest)
    for filename in RUNTIME_FILENAMES:
        if files[f"rollback/{filename}"].read_bytes() != canonical_document_bytes(runtime_documents[RUNTIME_FILENAMES.index(filename)]):
            raise PilotExecutionError("activation rollback document does not match current runtime")
    return {
        "activated": {filename: files[f"activated/{filename}"] for filename in RUNTIME_FILENAMES},
        "rollback": {filename: files[f"rollback/{filename}"] for filename in RUNTIME_FILENAMES},
    }


def validate_repository(repo: pathlib.Path, expected_commit: str, *, execute: bool) -> dict[str, Any]:
    ...
    branch = subprocess.run(
        ["git", "branch", "--show-current"], cwd=repo, check=False, capture_output=True, text=True
    )
    ...
    return {"commit": head.stdout.strip(), "branch": branch.stdout.strip(), "clean": clean, "exact": exact}


def validate_gateway_url(url: str) -> str:
    normalized = url.rstrip("/")
    if normalized not in {"http://127.0.0.1:8104", "http://localhost:8104", "http://[::1]:8104"}:
        raise PilotExecutionError("gateway URL is not the fixed loopback endpoint")
    return normalized


def prepare_evidence_directory(path: pathlib.Path, *, owner_uid: int) -> pathlib.Path:
    absolute = pathlib.Path(os.path.abspath(os.fspath(path)))
    ...
    if not stat.S_ISDIR(parent.st_mode) or parent.st_uid != owner_uid or stat.S_IMODE(parent.st_mode) & 0o022:
...
def copy_runtime_backup(runtime_paths: dict[str, pathlib.Path], evidence_dir: pathlib.Path) -> dict[str, pathlib.Path]:
...
def verify_hashes(paths: dict[str, pathlib.Path], expected: dict[str, str], label: str) -> None:
    for filename, path in paths.items():
        if sha256_bytes(path.read_bytes()) != expected[filename]:
            raise PilotExecutionError(f"{label} hash mismatch: {filename}")


def minimize_submission(status: int, response: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "provider_message_id_sha256": (
            sha256_bytes(str(provider_message_id).encode("utf-8")) if provider_message_id else None
        ),
        "response_sha256": sha256_bytes(canonical_document_bytes(response)),
        "audit_event_sha256": (
            sha256_bytes(canonical_document_bytes(audit_event)) if isinstance(audit_event, dict) else None
        ),
        "raw_response_stored": False,
    }


def audit_context(context: ExecutionContext, *, now: datetime | None = None, execute: bool = False) -> dict[str, Any]:
    current = datetime.now(timezone.utc) if now is None else now.astimezone(timezone.utc)
    repo = ROOT.resolve()
    repository = validate_repository(repo, context.expected_commit, execute=execute)
    validate_gateway_url(context.gateway_url)
    runtime_config, runtime_policy, runtime_identities, runtime_paths = validate_runtime_documents(context.runtime_root)
    authorization_uid = 0 if execute else os.geteuid()
    authorization_path = validate_private_file(
        context.authorization_path, "pilot authorization", owner_uid=authorization_uid
    )
    request_path = validate_private_file(context.request_path, "pilot request", owner_uid=authorization_uid)
    authorization = load_json(authorization_path, "pilot authorization")
    request = load_json(request_path, "pilot request")
    request_status = validate_request(request, authorization)
    manifest_path = validate_private_file(context.bundle_dir / "manifest.json", "activation bundle manifest")
    ...
    suppression_path = validate_private_file(
        ...
        owner_uid=0 if execute else None,
    )
...
def execute_pilot(
    ...
    *,
    ...
    if os.environ.get(EXECUTION_ENVIRONMENT_GATE) != "yes":
        raise PilotExecutionError("one-message execution authorization environment gate is absent")
    hostname = subprocess.run(
        ["hostname", "-f"], check=False, capture_output=True, text=True
    )
    if hostname.returncode != 0 or hostname.stdout.strip().casefold() != context.expected_host.casefold():
    ...
    source_hashes = {
        filename: sha256_bytes(path.read_bytes()) for filename, path in audit["runtime_paths"].items()
    }
    ...
    try:
        ...
        adapter.replace_runtime(audit["bundle_files"]["activated"], context.runtime_root)
        adapter.restart_service(context.service)
        health_status, health = adapter.get_json(context.gateway_url + "/outbound-mail/healthz")
        if health_status != 200 or health.get("status") != "ok":
            raise PilotExecutionError("activated gateway health check failed")
        send_attempted = True
        status, response = adapter.send_once(
            context.gateway_url + "/outbound-mail/send",
            audit["request"],
        )
        submission = minimize_submission(status, response)
        ...
        if activation_started:
            try:
                ...
                if rollback_health_status != 200 or rollback_health.get("status") != "ok":
                    raise PilotExecutionError("post-rollback gateway health check failed")
                restored = {
                    filename: sha256_bytes((context.runtime_root / filename).read_bytes())
                    for filename in RUNTIME_FILENAMES
                }
                if restored != source_hashes:
                    raise PilotExecutionError("post-rollback runtime hashes do not match preflight")
            except Exception as exc:
    ...
    record = {
        ...
        "authorization_sha256": activation_bundle.sha256_document(audit["authorization"]),
        "bundle_manifest_sha256": sha256_bytes(
            canonical_document_bytes(audit["manifest"])
        ),
        "pilot_recipient_sha256": audit["authorization"]["pilot_recipient_sha256"],
        "pilot_payload_sha256": audit["authorization"]["pilot_payload_sha256"],
        "smtp_auth_canary_sha256": audit["authorization"]["smtp_auth_canary_sha256"],
        ...
        "post_rollback_safe_disabled": activation_started and rollback_error is None,
        "failure": failure,
...
def audit_report(context: ExecutionContext) -> dict[str, Any]:
    ...
    return {
        ...
        "authorization_id": audit["authorization"]["authorization_id"],
        "authorization_sha256": activation_bundle.sha256_document(audit["authorization"]),
        "bundle_manifest_sha256": sha256_bytes(canonical_document_bytes(audit["manifest"])),
        "pilot_recipient_sha256": audit["authorization"]["pilot_recipient_sha256"],
        "pilot_payload_sha256": audit["authorization"]["pilot_payload_sha256"],
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--suppression-database", type=pathlib.Path, default=DEFAULT_SUPPRESSION_DATABASE)
