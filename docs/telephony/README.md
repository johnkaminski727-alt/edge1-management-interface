...
- disabled design-only protected runtime policy, read-only planner, and non-installable fail-closed systemd templates
...
The protected runtime design is deliberately non-executable. Its policy keeps deployment, scheduling, audit append, retention, and all host mutations false. Its templates remain under `design/`, contain no `[Install]` section, require a missing activation sentinel and intentionally absent runner, and therefore cannot activate the runtime.
...
python3 tests/validate_telephony_report_runtime_design.py
...
The report-audit and aggregate report-generator foundations are repository-complete. The disabled protected runtime design is also repository-complete. No runner, activation sentinel, runtime or evidence directory, report service, timer, retention job, automatic audit append, source collection, or runtime deployment is implemented or accepted.
...
7. implement a separately reviewed runner and rollback-capable installer only after explicit activation, audit-append, scheduling, and retention authority is established
