...
| `creekco.ca` | Namecheap shared hosting | Three `jellyfish.systems` MX hosts | Shared-hosting SPF present | `p=none` | Six role addresses passed limited round-trip tests and are registered |
...
All six are now represented in the inbound route table and automatic sender-selection map. In particular, `accessibility@creekco.ca` and `noc@creekco.ca` were reconciled into the 37-route registry on 2026-08-01. Their sender profiles remain live-disabled, and both route internally to `maildesk@ww.cx`.

The remaining configured CreekCo identities have not been proven individually. A provider export is still needed to distinguish real mailboxes from aliases or forwarders, including the provider-side object type for each of the six verified addresses.
...
3. Verify the provider-side mailbox, alias, or forwarder objects behind all six tested CreekCo identities.
