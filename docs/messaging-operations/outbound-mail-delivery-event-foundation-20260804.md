...
- tests: `tests/test_outbound_mail_delivery_events.py` and `tests/validate_outbound_mail_delivery_events.py`.
...
This module deliberately provides no unsuppress or delete operation. Any future removal of a suppression requires separate reviewed evidence, operator authorization, and an auditable workflow.
...
This foundation does not expose a webhook or listener, poll a mailbox, log into a provider, inspect credentials, store raw provider payloads, change a suppression manually, alter the gateway configuration, activate a sender or provider, prepare a message, or send a message.
