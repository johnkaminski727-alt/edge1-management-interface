...
- `https://edge1.ww.cx/admin/` resolves to the FreePBX Administration surface at `/admin/config.php`. This is a confirmed WAN-reachable `private-control` surface pending authenticated host inspection and exposure reduction.
- `https://edge1.ww.cx/ucp/` exposes the FreePBX User Control Panel login over public HTTPS. This is a confirmed WAN-reachable `private-control` surface pending authenticated host inspection and exposure reduction.
- Rendered FreePBX pages exposed session-adjacent/runtime and internal-network data. Those values are intentionally omitted from the durable evidence record.
- A narrow outside-in request to the known loopback-intended Operations API port `8097` did not finish loading. This is inconclusive and must not be treated as proof that the port is either closed or open from WAN.
...
- `https://ww.cx/admin/bigbird-operations-console.php` redirects an unauthenticated browser to the existing WW.CX sign-in page, confirming the established Operations Console route is deployed behind the existing authentication boundary.
...
4. Attribute every discovered listener to one of the five Control Surfaces classes; treat the currently public FreePBX Administration and UCP web surfaces as `private-control` unless live evidence proves a different architecture.
...
8. Remove ordinary WAN reachability to FreePBX Administration/UCP only after the current Apache/FreePBX routing, authentication/session behavior, WebSockets and dependent workflows are identified and a tested private replacement path is available.
9. Configure ordinary `edge1.ww.cx` HTTP(S) behavior to redirect to `https://creekco.ca/time/` only after current vhost/certificate/service-specific routing proves the redirect is safe.
10. Establish the approved Business159 execution path, run the documented WW.CX deployment dry run, preserve the document-root `wwcxjywl:nobody` / `0750` invariant, deploy `origin/main`, and verify the exact deployed revision.
11. Perform authenticated WW.CX browser acceptance for the Operations Center and Control Surfaces page, including authorization denial, diagnostics, stale/error rendering and secret-boundary checks.
12. Design/activate a temporary same-origin FreePBX session broker only after current FreePBX routing, cookies, WebSockets, CSP, `X-Frame-Options` and path behavior are inspected; prove it never opens the backend to WAN.
13. Keep AI integrations private, provider-independent and tool-allowlisted; no provider receives uncontrolled machine access.
14. Record before/after evidence, rollback, exact repository/deployed revisions and remaining blockers.
