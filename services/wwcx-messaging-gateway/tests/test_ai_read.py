...
def message_payload() -> dict[str, object]:
    return {
        ...
        "media": [
            {
                "url": "https://provider.invalid/private/object/123",
...
def test_single_event_read_uses_opaque_event_id() -> None:
    reset_store()
    accepted = client.post("/v1/simulator/messages", headers=SIM_HEADERS, json=message_payload())
...
def test_status_advertises_read_capabilities_not_send() -> None:
    response = client.get("/v1/management/status", headers=READ_HEADERS)
    assert response.status_code == 200
    data = response.json()
