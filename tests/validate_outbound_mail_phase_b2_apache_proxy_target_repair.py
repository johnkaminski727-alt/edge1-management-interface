#!/usr/bin/env python3
"""Validate the bounded Phase B2 Apache proxy-target repair package."""

from __future__ import annotations

import pathlib
import subprocess

ROOT = pathlib.Path(__file__).resolve().parents[1]
SCRIPT = ROOT / "deploy/messaging/repair-outbound-mail-phase-b2-apache-proxy-target.sh"
TEMPLATE = ROOT / "deploy/messaging/outbound-mail-preparation-api-apache.conf.example"
RUNBOOK = ROOT / "docs/messaging-operations/outbound-mail-apache-proxy-target-repair-20260804.md"

for path in (SCRIPT, TEMPLATE, RUNBOOK):
    assert path.is_file(), path

script = SCRIPT.read_text(encoding="utf-8")
for required in (
    "set -eu",
    "umask 077",
    "ACTION=${ACTION:-audit}",
    "APACHE_PROXY_TARGET_REPAIR_AUTHORIZED=${APACHE_PROXY_TARGET_REPAIR_AUTHORIZED:-no}",
    "ROLLBACK_EVIDENCE=${ROLLBACK_EVIDENCE:-}",
    "GIT_OPTIONAL_LOCKS=0 git -C \"$REPO_ROOT\" status --porcelain --untracked-files=all",
    "FRAGMENT_PATH=/etc/apache2/wwcx-outbound-mail-preparation-api.conf",
    "FAULTY_STATUS=",
    "FAULTY_PREPARE=",
    "REPAIRED_TARGET=",
    "apache-target-canary@example.invalid",
    "example.invalid",
    "delivery_disabled",
    "candidate changes more than the two proxy target URLs",
    "apache2ctl configtest",
    'systemctl reload "$APACHE_SERVICE"',
    "trap on_exit EXIT HUP INT TERM",
    "automatic_rollback=pass",
    "ready_for_explicit_apache_proxy_target_repair_authorization",
    "awaiting_business159_source_acceptance",
    "hmac_secret_read no",
    "provider_or_sender_enabled no",
    "external_delivery_enabled no",
    "message_prepared no",
    "message_sent no",
    "SHA256SUMS",
):
    assert required in script, required

for forbidden in (
    "systemctl restart",
    "systemctl stop",
    "systemctl start",
    "systemctl enable",
    "systemctl disable",
    "a2enmod",
    "a2ensite",
    "certbot",
    "openssl pkey",
    "openssl rsa",
    "WWCX_MAIL_GATEWAY_TOKEN=",
    "nft add",
    "iptables -A",
    "ufw allow",
    "nsupdate",
    'ProxyPassMatch "http://0.0.0.0',
    '/outbound-mail/send" retry=',
):
    assert forbidden not in script, forbidden

assert script.index("trap on_exit EXIT HUP INT TERM") < script.index(
    'install -o root -g root -m 0644 "$EVIDENCE_DIR/fragment.candidate.conf" "$FRAGMENT_PATH"'
)
assert script.index('apache2ctl configtest > "$EVIDENCE_DIR/configtest.txt"') < script.index(
    'systemctl reload "$APACHE_SERVICE" > "$EVIDENCE_DIR/apache-reload.txt"'
)
assert script.index("live fragment drifted after target repair") < script.index(
    'cp -a -- "$ROLLBACK_EVIDENCE/fragment.before.conf" "$FRAGMENT_PATH"'
)
assert script.count("record message_sent no") == 2
assert script.count("record external_delivery_enabled no") == 2
assert script.count("apache-target-canary@example.invalid") == 1

syntax = subprocess.run(["sh", "-n", str(SCRIPT)], cwd=ROOT, check=False)
assert syntax.returncode == 0

template = TEMPLATE.read_text(encoding="utf-8")
assert template.count('ProxyPassMatch "http://127.0.0.1:8104"') == 2
assert 'ProxyPassMatch "http://127.0.0.1:8104/outbound-mail/api/v1/status"' not in template
assert 'ProxyPassMatch "http://127.0.0.1:8104/outbound-mail/api/v1/prepare"' not in template
assert template.count("Require ip PREPARATION_CLIENT_CIDR") == 2
assert "Apache appends" in template
assert "duplicate the path" in template
assert "/outbound-mail/send" not in template

faulty = template.replace(
    '    ProxyPassMatch "http://127.0.0.1:8104" retry=0 connectiontimeout=5 timeout=30',
    '    ProxyPassMatch "http://127.0.0.1:8104/outbound-mail/api/v1/status" retry=0 connectiontimeout=5 timeout=30',
    1,
).replace(
    '    ProxyPassMatch "http://127.0.0.1:8104" retry=0 connectiontimeout=5 timeout=30',
    '    ProxyPassMatch "http://127.0.0.1:8104/outbound-mail/api/v1/prepare" retry=0 connectiontimeout=5 timeout=30',
    1,
)
old_status = '    ProxyPassMatch "http://127.0.0.1:8104/outbound-mail/api/v1/status" retry=0 connectiontimeout=5 timeout=30'
old_prepare = '    ProxyPassMatch "http://127.0.0.1:8104/outbound-mail/api/v1/prepare" retry=0 connectiontimeout=5 timeout=30'
new = '    ProxyPassMatch "http://127.0.0.1:8104" retry=0 connectiontimeout=5 timeout=30'
candidate = faulty.replace(old_status, new).replace(old_prepare, new)
assert candidate == template
changes = [(a, b) for a, b in zip(faulty.splitlines(), candidate.splitlines()) if a != b]
assert changes == [(old_status, new), (old_prepare, new)]

runbook = RUNBOOK.read_text(encoding="utf-8")
for required in (
    "Business159",
    "162.0.217.71",
    "HTTP `404`",
    "HTTP `401`",
    "original request URL is appended",
    "proxy_target_path_appended_twice",
    "ACTION=audit",
    "APACHE_PROXY_TARGET_REPAIR_AUTHORIZED=yes",
    "automatic rollback",
    "does not authorize or perform",
):
    if required == "proxy_target_path_appended_twice":
        assert required in script, required
    else:
        assert required in runbook, required

print("Outbound mail Apache proxy-target repair validation passed")
print("Exactly two complete backend targets are reduced to the fixed loopback origin")
print("Audit defaults, exact-commit checks, disabled-send proof, and automatic rollback are enforced")
print("Credentials, provider activation, delivery, and message traffic remain blocked")
