...
def main() -> int:
    ...
    assert subprocess.run([sys.executable, "-m", "py_compile", str(ACCEPTANCE)], check=False).returncode == 0
    assert subprocess.run([sys.executable, "-m", "py_compile", str(RENDERER)], check=False).returncode == 0
    ...
    print("Pipe delivery is one-recipient and preserves original-recipient evidence")
