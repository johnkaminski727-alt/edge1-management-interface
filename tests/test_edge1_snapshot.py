...
def test_commands_are_fixed_absolute_candidates_and_read_only_contract():
    ...
    assert "firewall" in MODULE.COMMANDS
...
def test_unknown_command_and_service_fail_closed():
    try:
        MODULE.run_fixed_command("shell")
    except KeyError:
        pass
    else:
        raise AssertionError("unknown command must be rejected")
    try:
        MODULE.service_state("user-controlled.service")
    except ValueError:
        pass
    else:
...
def test_digest_mode_never_returns_raw_output():
    ...
    with mock.patch.object(MODULE, "resolve_executable", return_value="/usr/sbin/nft"), \
         mock.patch.object(MODULE.subprocess, "run", return_value=completed):
        result = MODULE.run_fixed_command("firewall")
...
def test_collect_snapshot_is_self_describing_and_non_mutating():
    ...
    with mock.patch.object(MODULE, "run_fixed_command", return_value=fixed), \
         mock.patch.object(MODULE, "service_state", side_effect=lambda unit: {"unit": unit, "status": "unavailable"}), \
         mock.patch.object(MODULE, "config_digests", return_value=[]), \
         mock.patch.object(MODULE, "configured_timezone", return_value="UTC"), \
         mock.patch.object(MODULE, "uptime_seconds", return_value=123.0), \
         mock.patch.object(MODULE, "os_release", return_value={"id": "debian"}), \
         mock.patch.object(MODULE, "memory_summary", return_value={}):
    ...
    assert len(snapshot["services"]["relevant"]) == len(MODULE.SERVICE_UNITS)


def test_markdown_contains_machine_capture_identity_and_safety_flags():
    snapshot = {
        ...
        "services": {"relevant": [{"unit": "edge1-operator.service", "status": "unavailable"}]},
    }
