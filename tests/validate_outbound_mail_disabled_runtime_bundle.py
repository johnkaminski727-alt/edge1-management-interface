...
with tempfile.TemporaryDirectory() as temporary:
    ...
    first_key = next(iter(unsafe_identities["identities"]))
    unsafe_identities["outbound_activation_authorized"] = True
    unsafe_identities["identities"][first_key]["live_enabled"] = True
    unsafe_identities["sender_selection"]["live_sender_allowlist"] = [first_key]
    expect_error(
        "enabled sender",
        lambda: MODULE.build_bundle(
            live_like_config,
...
print("Provider, policy, sender, delivery, and send activation fail closed")
