...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        self.requests.append({"method": "GET", "path": self.path, "auth": self.headers.get("authorization")})
        self._json(200, {"status": "ready", "paused": False})

    def do_POST(self) -> None:
        length = int(self.headers.get("content-length", "0"))
        payload = json.loads(self.rfile.read(length) or b"{}")
        self.requests.append({"method": "POST", "path": self.path, "auth": self.headers.get("authorization"), "payload": payload})
...
class ClientTests(unittest.TestCase):
    ...
    def test_status_uses_read_credential(self) -> None:
        ...
        self.assertEqual(Handler.requests[0]["auth"], "Bearer read-secret")
    ...
    def test_pause_includes_actor_and_reason(self) -> None:
        ...
        self.assertEqual(Handler.requests[0]["auth"], "Bearer control-secret")
